...
const { updateCacheVersion } = require('../public/wtm/js/update-cache-version');
const { resetCacheVersion } = require('../public/wtm/js/reset-cache-version');

const execAsync = promisify(exec);

describe('Cache Version Management Scripts', () => {
  const SW_PATH = path.join(__dirname, '..', 'public', 'wtm', 'sw.js');
  ...
  describe('Command Line Execution', () => {
    test('should work when executed from command line', async () => {
      ...
      // Act
      const { stdout } = await execAsync('node public/wtm/js/update-cache-version.js', { cwd: path.join(__dirname, '..') });
