...
describe('Calendar Progress API - Error Handling & Edge Cases', () => {
  ...
  describe('Error Handling', () => {
    it('rejects empty payload', async () => {
      const res = await createAuthenticatedRequest(server, 'post', '/api/calendar-progress').send({});
    ...
    it('PUT returns 404 for non-existent entry', async () => {
      ...
      const res = await createAuthenticatedRequest(server, 'put', '/api/calendar-progress')
    ...
    it('DELETE returns 404 for non-existent entry', async () => {
      const nonExistentDate = "2099-12-31";
      const res = await createAuthenticatedRequest(server, 'delete', '/api/calendar-progress')
    ...
    it('POST returns 409 for duplicate entries', async () => {
      ...
      const firstRes = await createAuthenticatedRequest(server, 'post', '/api/calendar-progress')
      ...
      const duplicateRes = await createAuthenticatedRequest(server, 'post', '/api/calendar-progress')
