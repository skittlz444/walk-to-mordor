 ...
 */
export async function handleRegister(request: Request, db: DbClient, env: Env, body: any) {
  ...
  try {
    ...
    const friendCode = await generateUniqueFriendCode(db.write);
 ...
 */
export async function handleSessionValidation(request: Request, db: DbClient, env: Env) {
 ...
 */
export async function validateSession(request: Request, db: DbClient, env?: Env): Promise<
 ...
 */
export async function validateAdminSession(request: Request, db: DbClient, env?: Env): Promise<
  | { valid: true; userId: number; isAdmin: true }
  | { valid: false; error: Response }
> {
  // First validate the session using existing logic
  const sessionResult = await validateSession(request, db, env);
 ...
 */
export async function handleUpdateProfile(request: Request, db: DbClient, env: Env, body: any) {
  // Validate session
  const sessionValidation = await validateSession(request, db, env);
 ...
 */
export async function handleGetAvatars(request: Request, db: DbClient, env: Env) {
  const sessionValidation = await validateSession(request, db, env);
  if (!sessionValidation.valid) {
    return sessionValidation.error;
  }
 ...
 */
export async function handleUpdatePreferences(request: Request, db: DbClient, env: Env, body: any) {
  // Validate session
  const sessionValidation = await validateSession(request, db, env);
  if (!sessionValidation.valid) {
    return sessionValidation.error;
 ...
 */
export async function handlePasswordResetRequest(request: Request, db: DbClient, env: Env, body: any) {
  const { email } = body || {};
 ...
 */
export async function handleResendConfirmation(request: Request, db: DbClient, env: Env, body: any) {
  const { email } = body || {};
