...
4. `leave_distance_behavior` defaults to 'keep' if not provided. Must be 'keep' or 'remove' if provided.
5. Generate a unique, cryptographically secure/non-enumerable 8-character alphanumeric invite code.
6. Set the creator as the leader in the `party_members` table and record `distance_at_join` equal to the user's current total distance, and `last_viewed_distance` = 0.
7. Return the newly created party details, including the generated invite code and configured settings (`distance_mode`, `leave_distance_behavior`).
8. Return a 401 Unauthorized status if the request is not authenticated.
9. Users can create multiple parties — no restriction on the number of parties a user can lead.
...
- [ ] Task 2: Input Validation & Invite Code Generation (AC: 2, 3, 4, 5)
