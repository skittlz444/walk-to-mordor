...
*   **Multi-Party UI:** Journey and Map pages include a party selector (hidden if user has no parties) to toggle between personal distance and any party's combined distance. Navigation drawer includes link to Fellowships page. The Fellowships UI uses a 3-page flow: Fellowships list (select or create) → Fellowship detail (members, contributions, total progress, last milestone crossed) → Fellowship management (leader only — settings, kick, transfer, invite code).
